...
#!/usr/bin/env python
# -------------------------------------------------
# SpreadNet Fire-Spread Streamlit App  — Fix B (wind-coupled training)
# -------------------------------------------------
import streamlit as st, tensorflow as tf, numpy as np, rasterio, requests, tempfile, os, math, random
import matplotlib.pyplot as plt
from matplotlib.cm import get_cmap
...
def wind_align(a,b): return math.cos(math.radians(a-b))
def direction_deg(y,x,ny,nx): return math.degrees(math.atan2(nx-x,ny-y))%360
def predict(net,ft): return net(np.asarray(ft,'float32'),training=False).numpy().ravel()

# ───────────────────────── UI ─────────────────────────
st.title("🔥 SpreadNet (vB wind-coupled)")
MOIST_GLOBAL = st.slider("Global Moisture (%)",0.0,40.0,1.0,.1)
WIND_SPEED   = st.slider("Wind Speed (m/s)",0.0,30.0,10.0,.1)
WIND_DIR_DEG = st.slider("Wind Direction (°)",0,359,250,1)
STEP_MIN     = st.slider("Time Step (min)",1,60,10,1)
MAX_SIM_MIN  = st.slider("Max Simulation Time (min)",60,5000,1440,10)
...
if st.button("Run Simulation"):
    ...
    try:
        ...
        # ─────────────── train wind-coupled dummy net ───────────────
        def make_sample(lbl):
            fc=random.choice(VALID_FUELS)
            emb=FUEL_EMB[fc]
            sl=random.uniform(0,60)
            mo=random.uniform(0,40)
            wd=random.uniform(0,30)
            al=random.uniform(-1,1)
            wa=(wd/30)*al                                  # wind-scaled align
            dist=random.choice([0,1])
            x=emb+[sl/60,mo/40,wd/30,al,wa,dist]           # 9-feature vector
            if lbl is None:                                # auto label by rule
                lbl=int( (wd>5 and al>0.2) or (sl>25 and mo<15) )
            return x,lbl

        def gen_data(n=60_000):
            X,Y=[],[]
            for _ in range(n):
                xi,yi=make_sample(None)
                X.append(xi);Y.append(yi)
            return np.array(X,'float32'),np.array(Y,'float32')

        tf.keras.backend.clear_session()
        net=build_net(); net.compile(optimizer='adam',loss='binary_crossentropy')
        Xtr,Ytr=gen_data(); net.fit(Xtr,Ytr,epochs=5,batch_size=2048,verbose=0)
        ...
        while burn.any() and minutes<MAX_SIM_MIN:
            ...
            for y,x in zip(*np.where(burn==1)):
                ...
                for dy,dx in NEIGH:
                    ...
                    if not(0<=ny<rows and 0<=nx<cols): continue
                    if burn[ny,nx]!=0 or fuel[ny,nx] in [93,98,99]: continue
                    al=wind_align(WIND_DIR_DEG,direction_deg(y,x,ny,nx))
                    wn=WIND_SPEED/30
                    wa=wn*al
                    feats.append(FUEL_EMB[int(fuel[ny,nx])]+[
                        slope[ny,nx]/60,MOIST_GLOBAL/40,wn,al,wa,
                        (DIAG if dy*dx else CELL)/DIAG])
                    cells.append((ny,nx))
            if feats:
                probs=predict(net,feats)
        ...
        cbar=fig.colorbar(im,ax=ax,ticks=[1,len(runs)])
        cbar.ax.set_yticklabels([f"{runs[0][0]} min",f"{runs[-1][0]} min"])
        st.pyplot(fig); st.success("Simulation complete!")
    finally:
        for p in (slope_path,fuel_path):
            if p and os.path.exists(p):
                try: os.remove(p)
